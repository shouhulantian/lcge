...
class IKBCOptimizer(object):
    ...
    def epoch(self, examples: torch.LongTensor):
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            b_begin = 0
            while b_begin < examples.shape[0]:
                time_range = actual_examples[b_begin:b_begin + self.batch_size].to('cuda' if torch.cuda.is_available() else 'cpu')

                ## RHS Prediction loss
                sampled_time = (
                        torch.rand(time_range.shape[0]).to('cuda' if torch.cuda.is_available() else 'cpu') * (time_range[:, 4] - time_range[:, 3]).float() +
                        time_range[:, 3].float()
                ).round().long()
                ...
                if self.model.has_time():
                    ...
                    truth = (
                            torch.rand(these_examples.shape[0]).to('cuda' if torch.cuda.is_available() else 'cpu') * (these_examples[:, 4] - these_examples[:, 3]).float() +
                            these_examples[:, 3].float()
                    ).round().long()
                    time_predictions = self.model.forward_over_time(these_examples[:, :3].to('cuda' if torch.cuda.is_available() else 'cpu').long())
                    time_loss = loss(time_predictions, truth.to('cuda' if torch.cuda.is_available() else 'cpu'))
